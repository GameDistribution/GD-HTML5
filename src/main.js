...
class SDK {
  ...
  constructor(options) {
    ...
    new Image().src = `https://ana.tunnl.com/event?page_url=${encodeURIComponent(
      this.referrer
    )}&game_id=${this.options.gameId}&eventtype=${1}`;
    ...
    this.gameDataPromise = new Promise(resolve => {
      ...
      const gameDataUrl = `https://game.api.gamedistribution.com/game/get/${gameData.gameId.replace(
        /-/g,
        ""
      )}/?domain=${parentDomain}&localTime=${new Date().getHours()}&v=${
        PackageJSON.version
      }`;
  ...
  _onEvent(event) {
    ...
    // TODO: It will be disabled because of downgrade of google analytics account.
